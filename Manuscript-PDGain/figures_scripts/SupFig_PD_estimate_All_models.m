...
%% Simulate PD
%there are many PD type and shape we can try see inside mrQ_simulate_PD
...
%% coils corlation  regularization
...
CV_ridgre=(calccod(PD_ridgre(:),PD(:))/100).^2

%for visualization clip to of to 100% error
PD_ridgre(PD_ridgre<0)=0;
PD_ridgre(PD_ridgre>2)=2;
...
%%
%PD_cor; PD_ridgre;  PD_T1reg; PD_Noreg;
mrvNewGraphWin
hold on
plot(PD(:),PD_Noreg(:),'.')
plot(PD(:),PD_cor(:),'.g')
...
legend( ['PD estimate without Reg R2 = ' num2str(CV_Noreg)]...
    ...
    ,['PD estimate Ridge R2 = ' num2str(CV_ridgre)],...
    ['PD estimate with T1 reg R2 = ' num2str(CV_T1reg)],...
    'Location','North')
%%

showMontage((PD_Noreg-PD)./PD);title('PD pracent error  no regretion')

showMontage((PD_cor-PD)./PD); title('PD pracent error corr')

showMontage((PD_ridgre-PD)./PD); title('PD pracent error ridge regretion')

showMontage((PD_T1reg-PD)./PD);title('PD pracent error T1 regretion')
