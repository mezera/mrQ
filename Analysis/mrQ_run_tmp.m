    ...
    if (mrQ.SEIR_done==0);
        if ~isfield(mrQ,'ArrangeSEIR_Date');
            ...
            if ~notDefined('inputData_seir')
                mrQ = mrQ_arrangeSEIR_nimsfs(mrQ,inputData_spgr);
            else
        ...
        % fitiing SEIR T1 map and register to SPGR
...
%% VIII. Create the synthetic T1-weighted images and save them to disk
...
%% XI. Calculate VIP, TV,  SIR and synthetic T1w
...
%% XII. Organize the OutPut directory
