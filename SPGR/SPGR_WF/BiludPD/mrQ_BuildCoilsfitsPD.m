function [PDcombineInfo ]=mrQ_BuildCoilsfitsPD(outDir,SunGrid,proclass,exsistFit,M0cfile,sqrtF,GEDIr)
%
...
if (exist(PDfile2,'file')) % if this file exsist we not gona do everthing again. if you like to redo remove this file first
    str=['the file: \n' PDfile2 ' \n exsit, therfore skipping building the PD image again. \n to redo the PD build remove the file \n' PDfile2 ' \n'];
    fprintf(str);
        ...
        if length(regexp(list, '.mat'))>=fNum,
            ...
            if (isempty(result) && tt>60)
                ...
                if length(find(reval))>0
                        ...
                        eval(['!rm -f ~/sgeoutput/*' sgename '*'])
                        if proclass==1
                         %   sgerun2('FitM0_sanGrid_v2(opt,jumpindex,jobindex);',[sgename num2str(ceil(rand(1)*10))],1,reval,[],[],8000);% we run the missing oupput again
                            % thesgerun2 usi only sqeuntial jobs so if we
                            % have only a few we will re run them one by
                            % one
                                                     for kk=1:length(reval)
                                                        sgerun2('FitM0_sanGrid_v2(opt,jumpindex,jobindex);',[sgename num2str(kk)],1,reval(kk),[],[],8000);% we run the missing oupput again
                                                  end
                        else
                            sgerun('FitM0_sanGrid_v2(opt,jumpindex,jobindex);',sgename,1,reval,[],[],8000);% we run the missing oupput again
                        end
                    else
                ...
                %                 sgerun('FitM0_sanGrid_v2(opt,jumpindex,jobindex);',sgename,1,reval,[],[],3000);% we run the missing oupput again
                %
            else
                %  keep waiting
            end
...
for jj=1:length(wh1),
   ...
   %Val=box.Val./median(box.Val(:));
   ResVal=ResVal./median(Val(:));
