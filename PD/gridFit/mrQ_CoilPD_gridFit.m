...
for ii= st:ed,
    ...
    if  skip(Iter)==1
        ...
        [X_valdationErrF,  X_gEstF]=pdX_valdationLoop_1(opt.lambda,opt.Kfold,M0_v(:,opt.Clist), pBasis,R1basis,g0,Segmask);
        ...
        if min(X_valdationErrF(2,:))*2 > X_valdationErrF(2,end)
            ...
            [PDfit,~,G,gEst(:,:,Iter), resnorm(Iter),exitflag(Iter) ]=pdCoilSearch_T1reg( opt.lambda(best2),M0_v(:,opt.Clist),pBasis,R1basis,X_gEstF(:,:,1,best2),Segmask);
...
save(name,'gEst','resnorm','exitflag','st','ed','skip','fb' ,'X_valdationErr','X_valdationErrSN')
