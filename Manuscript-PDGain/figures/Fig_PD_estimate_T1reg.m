...
%% Ridge reg (not helpful)
kFold = 2;
lambda1= [1e4 5e3 1e3 5e2 1e2 5e1 1e1 5  1e0 0.5 1e-1 0];
[X_valdationErr, gEstT, resnorm, FitT useX, kFold ]= ...
    pdX_valdationRidgeLoop(lambda1,kFold,MR_Sim.M0SN,phantomP.pBasis);

% mrvNewGraphWin;
% semilogy(lambda1,X_valdationErr(2,:),'*-');         
% xlabel('lambda');ylabel('CV error');X_valdationErr(2,:)./min(X_valdationErr(2,:))
