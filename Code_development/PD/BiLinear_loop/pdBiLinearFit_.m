function OutPut = pdBiLinearFit_3(M0_v,pBasis,Lambda,maxLoops,sCriterion,PD,plotFlag,TruePar,D,HoldforCV,W,altenateCoils)
...
%altenateCoils if true will fit PD on hulf of the data and G on the other.
%
...
if notDefined('altenateCoils'),  altenateCoils=false; end
...
for ii=1:nCoils
    ...
    g0(:,ii) = pBasis(Fitvox,:) \ G(Fitvox,ii);  % Polynomial approximation
end
...
while tryagain == 1
    ...
    if altenateCoils
    Coils=randperm(nCoils);
    useCoil=Coils(1:round(nCoils/2));
   
    end
    % fit linear equation coefficients using ridge regression
    g = RidgeRegressCoilfit(PD(Fitvox), Lambda, M0_v(Fitvox,useCoil), pBasis(Fitvox,:),D,W);
...
%    [PDn, Gn] = pdEstimate(M0_v(Fitvox,:), pBasis(Fitvox,:), g);
        [PDn, Gn] = pdEstimate(M0_v(:,useCoil), pBasis(:,:), g);  %as we solve voxel by voxel we can run on all of them
...
%     PDn = PDn ./ PDn(1);
    ...
    M0n = Gn.*repmat( PDn,1,size(Gn,nCoils);
