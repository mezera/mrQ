function [AnalysisInfo]=mrQ_ANTS_warp_EPI2SPGR(AnalysisInfo,t1fileHM,outDir,B1file)
%
...
AnalysisInfo.WARP_EPI_SPGR=fullfile(outDir,'WARP_EPI_SPGR');
...
%if ~(exist([AnalysisInfo.RB_WARP_EPI_SPGR 'Warp.nii.gz'],'file') && exist([AnalysisInfo.RB_WARP_EPI_SPGR 'Affine.txt'],'file') )
    ...
    cmANTS = ['xterm -e ANTS 3 -m CC[' t1fileHM ',' AnalysisInfo.T1_spgr_epi ',1,2]  -o ' AnalysisInfo.WARP_EPI_SPGR '.nii.gz --rigid-affine true'];
    ...
    if status ~=0
        cmANTS=['ANTS 3 -m CC[' t1fileHM ',' AnalysisInfo.T1_spgr_epi ',1,2]  -o ' AnalysisInfo.WARP_EPI_SPGR '.nii.gz --rigid-affine true'];
        [status, result] = system(cmANTS,'-echo')
    end
...
%cmWarp=['xterm -e WarpImageMultiTransform  3 ' AnalysisInfo.T1_spgr_epi ' '  AnalysisInfo.B1_epi_spgr ' -R '  B1file  ' ' AnalysisInfo.WARP_EPI_SPGR 'Warp.nii.gz ' AnalysisInfo.WARP_EPI_SPGR 'Affine.txt'];
    ...
    cmWarp=['xterm -e WarpImageMultiTransform  3 ' B1file ' '  AnalysisInfo.B1_epi_spgr ' -R '  t1fileHM  ' ' AnalysisInfo.WARP_EPI_SPGR 'Warp.nii.gz ' AnalysisInfo.WARP_EPI_SPGR 'Affine.txt'];
    % Run the command in unix and get back status and results:
    [status, result] = system(cmWarp);
    if status ~=0
        cmWarp=['WarpImageMultiTransform  3 ' B1file ' '  AnalysisInfo.B1_epi_spgr ' -R '  t1fileHM  ' ' AnalysisInfo.WARP_EPI_SPGR 'Warp.nii.gz ' AnalysisInfo.WARP_EPI_SPGR 'Affine.txt'];
        % Run the command in unix and get back status and results:
        [status, result] = system(cmWarp);
    end
...
AnalysisInfo.T1_epi_spgr=fullfile(outDir,'Warp_T1_EPI2SPGR.nii.gz');
    ...
    % ANTs: make the ANTs command
    cmWarp=['xterm -e WarpImageMultiTransform  3 ' AnalysisInfo.T1_spgr_epi ' '  AnalysisInfo.T1_epi_spgr ' -R '  t1fileHM  ' ' AnalysisInfo.WARP_EPI_SPGR 'Warp.nii.gz ' AnalysisInfo.WARP_EPI_SPGR 'Affine.txt'];
    % Run the command in unix and get back status and results:
    [status, result] = system(cmWarp);
    if status ~= 0
        cmWarp=['WarpImageMultiTransform  3 ' AnalysisInfo.T1_spgr_epi ' '  AnalysisInfo.T1_epi_spgr ' -R '  t1fileHM  ' ' AnalysisInfo.WARP_EPI_SPGR 'Warp.nii.gz ' AnalysisInfo.WARP_EPI_SPGR 'Affine.txt'];
