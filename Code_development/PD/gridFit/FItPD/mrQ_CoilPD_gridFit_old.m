...
for ii= st:ed,
    ...
    clear M01  t1  BM1  SZ M0_v R1basis PDinit Segmask g0 G0 mask1 X_valdationErrF   X_gEstF  best2 seg
    Iter= Iter+1;
    tic
    ...
    [M01, t1, BM1, SZ, skip(Iter)]= mrQ_GetM0_boxData(opt,T1,M0,BM,fb(Iter,1,:),smoothkernel);
    ...
    if  skip(Iter)==1
        ...
        %% segment the box by T1
        
        [Seg, C]= mrQ_localT1Seg(R1(BM1)) ;
        ...
        for ii=1:nCoils
            G(mask1,ii)  = M0_v(mask1,opt.Clist(ii)) ./ PDinit(mask1);         % Raw estimate
        ...
        %%  X-Validation Fit of coil Gain with T1 regularization
                ...
                [X_valdationErrF,  X_gEstF]=pdX_valdationLoop_2(opt.lambda,3,M0_v(:,opt.Clist), pBasis,R1basis,g0,Segmask);
...
%       [X_valdationErrF,  X_gEstF]=pdX_valdationLoop_2(opt.lambda,opt.Kfold,M0_v(BM1,opt.Clist), pBasis(BM1,:),R1basis(BM1,:),g0,Segmask(BM1));
      ...
      % if there is not enght data we many X_valdation Err are
        ...
        if min(X_valdationErrF(2,:))*2 > X_valdationErrF(2,end)
            ...
            [PDfit,~,G,gEst(:,:,Iter), resnorm(Iter),exitflag(Iter) ]=pdCoilSearch_T1reg( opt.lambda(best2),M0_v(BM1,opt.Clist),pBasis(BM1,:),R1basis(BM1,:),X_gEstF(:,:,1,best2),Segmask(BM1));
...
save(name,'gEst','resnorm','exitflag','st','ed','skip','fb' ,'X_valdationErr','X_valdationErrSN','BestReg')
