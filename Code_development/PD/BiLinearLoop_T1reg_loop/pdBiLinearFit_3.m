function OutPut = pdBiLinearFit_3(M0_v,pBasis,Lambda1,maxLoops,sCriterion,PD,plotFlag,TruePar,D,HoldforCV,W,Lambda2,R1)
%OutPut = pdBiLinearFit_3(M0_v,pBasis,Lambda1,maxLoops,sCriterion,PD,plotFlag,TruePar,D,HoldforCV,W,Lambda2,R1)
...
%  Lambda1        A ridge regration regularization term
...
%,Lambda2      Lambda2 the wighting on R1 regularisation (a single value).
%R1                   the musred R1 values (1/T1) that will be used to
%                       regularized PD to be linear function  (nVoxels x 1).
%
...
while tryagain == 1
    ...
    g = RidgeRegressCoilfit(PD(Fitvox), Lambda1, M0_v(Fitvox,:), pBasis(Fitvox,:),D,W);
...
[PDn, Gn] = pdEstimateT1Reg(M0_v, pBasis, g,Lambda2,R1)
