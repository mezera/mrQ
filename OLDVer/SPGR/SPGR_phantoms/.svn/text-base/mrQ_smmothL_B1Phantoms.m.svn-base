function [B1,AnalysisInfo]=mrQ_smmothL_B1Phantoms(B1epifile,outDir,B1file,xform,degree,AnalysisInfo,t1fileHM,B1epiResidfile,Tmask);
%
...
tisuuemask=  B1<1.5 & B1>.5 & ~isinf(B1fitResid) ;

% no for big residual

tisuuemask=tisuuemask & B1fitResid<prctile(B1fitResid(find(B1fitResid)),97) ;
...
dtiWriteNiftiWrapper(single(tmp1), SE_Xform, B1file);
