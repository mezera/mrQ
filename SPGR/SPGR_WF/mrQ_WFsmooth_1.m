 ...
 %lets calibrate the pd by the pd of the csf roi
   ...
   T1file = fullfile(outDir,'T1_map_lsq.nii.gz');
    if(exist(T1file,'file'))
        disp(['try to Load  ' T1file '...']);
        T1 = readFileNifti(T1file);
        T1=T1.data;
        
    else
        T1file = fullfile(outDir,'maps/T1_map_lsq.nii.gz')
        if(exist(T1file,'file'))
            disp(['try to Load  ' T1file '...']);
            T1 = readFileNifti(T1file);
            T1=T1.data;
    ...
    % we use isosurface T1 to remove bias in PD map we use only high T1
    % T1>2.5. To assure we ue only water voxels. that will minimize the
    % effect on the T1 /PD ratio information
    
    G=mrQ_PDT1resideBias(T1,pd_corect,[],2);
    pd_corect=pd_corect./G;
    
   
% let cut outlayers
 %lets check that the CSF value didn't change
    [d dd]= ksdensity(pd_corect(find(CSF1)), [min(pd_corect(find(CSF1))):0.001:max(pd_corect(find(CSF1)))] );
% %  CalibrationVal=mean(PD(find(CSF)));
 CalibrationVal= dd(find(d==max(d)));% median(PD(find(CSF)));
