function [s,xform] = relaxAlignAllcomplex(s, refImg, outMm, bb, interp)
%
% [s,xform] = relaxAlignAllcomplex(s, refImg, [outMm=refImg.pixdim], [bb=true], [interp=7])
%
...
%2011 jun 27 Aviv Mezer add a compex data analysis to the standrd RFD
%relaxAlignAll.m
%. this assume that the data is compex align the abs data and transform
%the real and imagenery part and then make the new compex image
%of 
...
nSeries = numel(s);
for(ii=1:nSeries)
sR(ii).im=real((s(ii).imData));
sImag(ii).im=imag((s(ii).imData));
    s(ii).imData=abs(s(ii).imData);
...
for(ii=1:nSeries)
    ...
    [s(ii).imData,s(ii).mmPerVox] = applyCannonicalXform(s(ii).imData, canXform, s(ii).mmPerVox, false);
        [sR(ii).im] = applyCannonicalXform(sR(ii).im, canXform, s(ii).mmPerVox, false);
        [sImag(ii).im] = applyCannonicalXform(sImag(ii).im, canXform, s(ii).mmPerVox, false);
...
if(~isempty(refImg))
    ...
    for(ii=1:nSeries)
        ...
        [sImag(ii).im,dd] = mrAnatResliceSpm(double(sImag(ii).im), xf, bb, outMm, interp, 0);
        [sR(ii).im,dd] = mrAnatResliceSpm(double(sR(ii).im), xf, bb, outMm, interp, 0);
        s(ii).mmPerVox = outMm;
    end
else
    ...
    for(ii=1:nSeries)
        [s(ii).imData,s(ii).imToScanXform] = mrAnatResliceSpm(double(s(ii).imData), s(ii).xformToFirst, bb, outMm, interp, 0);
                [sR(ii).im,s(ii).imToScanXform] = mrAnatResliceSpm(double(sR(ii).im), s(ii).xformToFirst, bb, outMm, interp, 0);
                        [sImag(ii).im,s(ii).imToScanXform] = mrAnatResliceSpm(double(sImag(ii).im), s(ii).xformToFirst, bb, outMm, interp, 0);
...
xform = s(1).imToScanXform;
    ...
    for(ii=1:nSeries)
s(ii).imData=sR(ii).im+i*sImag(ii).im;
    end;
